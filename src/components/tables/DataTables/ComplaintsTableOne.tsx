...
export default function ComplaintsTableOne({
    ...
    const renderComplaintDetails = () => {
        ...
        return (
            <div className="p-4">
                <h3 className="text-lg font-bold mb-4">Détails de la plainte </h3>
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        {/* <p><strong>ID :</strong> {selectedComplaint.id}</p> */}
                        <p><strong>Description :</strong> {selectedComplaint.description}</p>
                        <p><strong>Statut :</strong> {selectedComplaint.status}</p>
                        <p><strong>Date de création :</strong> {new Date(selectedComplaint.createdAt).toLocaleDateString()}</p>
                        <p><strong>Date de la plainte :</strong> {selectedComplaint.complaintDate}</p>
                        <p><strong>Heure de la plainte :</strong> {selectedComplaint.complaintTime}</p>
                        <p><strong>Temps perdu :</strong> {selectedComplaint.tempsPerdu}</p>
                        <p><strong>Montant :</strong> {selectedComplaint.montant} {selectedComplaint.TransportForm.currency}</p>
                    </div>
                    <div>
                        <p><strong>Utilisateur :</strong> {selectedComplaint.User.firstname ?? ''} {selectedComplaint.User.lastname} ({selectedComplaint.User.email})</p>
                        <p><strong>Pays :</strong> {selectedComplaint.Country.name}</p>
                        <p><strong>Région :</strong> {selectedComplaint.Region.name}</p>
                        <p><strong>Type de plainte :</strong> {selectedComplaint.ComplaintType.name}</p>
                        <p><strong>Service :</strong> {selectedComplaint.Service.name}</p>
                        <p><strong>Numéro de chargement :</strong> {selectedComplaint.loadingNumber}</p>
                        <p><strong>Produit :</strong> {selectedComplaint.TransportForm.productLabel} ({selectedComplaint.TransportForm.productQuantity} unités)</p>
                    </div>
                </div>
