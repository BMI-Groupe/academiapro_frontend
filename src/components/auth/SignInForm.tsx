  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!req || req.success === false) {
        ...
        let errorMessage = req?.message || "Une erreur est survenue lors de la connexion.";
        
        if (req?.status === 401) {
          errorTitle = "Identifiants invalides";
          errorMessage = req.message || "Le numéro de téléphone ou le mot de passe est incorrect.";
        } else if (req?.status === 0) {
          errorTitle = "Erreur de connexion";
          errorMessage = "Impossible de contacter le serveur. Veuillez vérifier votre connexion.";
        } else if (req?.status >= 500) {
          errorTitle = "Erreur serveur";
          errorMessage = "Une erreur interne est survenue. Veuillez réessayer ultérieurement.";
        }
